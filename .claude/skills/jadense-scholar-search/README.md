...
- **Response caching** — provider responses and OpenAlex enrichment merges are reused per key for 168 hours; every cell reports `cacheHit`, the data's own collection date, and a `{ candidates, totalAvailable }` receipt.
- **Retrieval hardening** — per-source throttling and concurrency caps, one bounded same-cell retry for transient failures, per-cell `totalAvailable`/`capHit` diagnostics with cap handling split by source type, and `date_unknown` marks instead of silently dropping undated candidates.
...
// response.queryPlan -> providers, limit, dateRange, hit counts, per-cell receipts (totalAvailable, capHit, capPolicy, date counts, attempts)
...
| `--cache-dir <path>` | Response-cache directory (default `<package>/.cache`) |
| `--no-cache` | Do not read or write the response cache |
| `--refresh` | Bypass existing cache entries and fetch fresh data |
| `--user-agent <text>` | Injected HTTP User-Agent |
| `--contact-email <email>` | Provider contact metadata |

#### Response caching

Provider responses are cached one cell per `provider + normalized query + limit + date range + offline flag`, and OpenAlex enrichment merges one cell per `DOI ?? external id ?? normalized title + provider set`. The window is 168 hours from the cell's own collection moment; hits never extend it, only successful cells are stored (provider errors are re-recorded as diagnostics every run), and `--offline` runs neither read nor write the cache.

Every `queryPlan.queryStatuses[]` cell carries `cacheHit` and `collectedAt` (the data's own collection date — not the rerun date), and `queryPlan.enrichCacheHits[]` reports the same per enrichment hit, so a replay can record the data date instead of reporting week-old provider data as freshly fetched.

The cache directory (`--cache-dir`, `SCHOLAR_SEARCH_CACHE_DIR`, else `<package>/.cache`) must be a gitignored, non-delivery path: the default qualifies, a `workspace/<delivery>/…` directory or any tracked path does not (cleanup rules: `workspace-guide.md` §15).

A search cell stores `{ candidates, totalAvailable }` — the candidates from before date filtering plus the only reading that cannot be recomputed from them (date counts, `hitCount`, `capHit` and `capPolicy` are recomputed on a hit, so a cached cell keeps its truncation disclosure). `CACHE_VERSION` is unchanged: the stricter payload guard sends an old-shape cell (a bare candidate array) through the existing “shape mismatch = miss” path, so the cell is refetched and rewritten in the current shape — there is no compatibility branch. On a hit the cell reports `attempts: 0`, `retried: false`, `recoveredByRetry: false`, and `collectedAt` = the original collection moment.

#### Retrieval hardening

Every provider request passes through one shared gate, so a transient outage cannot silently shrink a cell and a truncated cell cannot read as complete.

- **Throttling and concurrency caps** — PubMed serial at ≤3 rps (three concurrent calls reliably produce 429s), arXiv ≤1 request / 3 s on a single connection (Terms of Use), OpenAlex and Crossref 10 rps, Semantic Scholar and SerpApi 1 rps. Retries and cap compensation queue through the same gate.
- **Bounded same-cell retry** — one in-place re-run per cell, only for `unavailable` (transport failure or 5xx) and `quota_exhausted` (429); `invalid_request` (400) and `auth_failed` (401/403) are never retried. Backoff is 800–1600 ms (exponential base 800 ms with jitter), stays inside the cell, never restarts the pool, and never enters the cache key.
- **Total-available diagnostics** — `totalAvailable` is `null` when a source reports no total (never 0 and never the returned count); `capHit` is `true` when `totalAvailable > hitCount` and `null` when the total is unknown, and `null` never reads as `false`.
- **Cap handling by source type** — exact-set sources (PubMed, arXiv) raise the cell limit to `min(totalAvailable, 30)` and then slice a dated window by year (oldest first, ≤6 slices of ≤30), recording whatever gap is left; recall-form sources (OpenAlex, Crossref, Semantic Scholar, Google Scholar) record the total only. `capPolicy` = `none` / `unknown` / `recorded_only` / `raised_limit` / `year_sliced`.
- **Date windows** — a candidate without a valid `YYYY-MM-DD` date is kept and marked `sourceMetadata.windowStatus = "date_unknown"`; a valid date outside the window is dropped and counted (`dateRejectedCount`); without a window every pooled candidate is `unfiltered`. A merged cluster drops the `date_unknown` mark as soon as one member is dated and in-window.
...
| `SCHOLAR_SEARCH_CACHE_DIR` | any provider | Response-cache directory; `--cache-dir` overrides it |
...
- **响应缓存** — provider 响应与 OpenAlex 回填合并结果按格复用、窗口 168 小时；每格带 `cacheHit`、数据自身采集日期与 `{ candidates, totalAvailable }` 收据。
- **取数加固** — 逐源节流与并发上限、瞬时失败同格有界重试一次、逐格 `totalAvailable`／`capHit` 诊断与分源类型的 cap 处置、无日期候选带 `date_unknown` 标记入池而不静默丢弃。
...
| `--cache-dir <path>` | 响应缓存目录（默认 `<包目录>/.cache`） |
| `--no-cache` | 不读也不写响应缓存 |
| `--refresh` | 越过已有缓存格重新取数 |
...
provider 响应按 `provider ＋ 归一查询 ＋ limit ＋ 日期窗 ＋ 离线标志` 一格一缓存，OpenAlex 回填合并结果按 `DOI ?? 外部标识 ?? 归一题名 ＋ provider 集合` 一格一缓存。窗口 168 小时、到期基准为该格数据自身采集时刻，命中不顺延；只缓存成功格（provider 错误不落缓存、诊断每次重记）；`--offline` 一跑既不读也不写。

`queryPlan.queryStatuses[]` 每格带 `cacheHit` 与 `collectedAt`（数据自身采集日期，不是重跑日期），`queryPlan.enrichCacheHits[]` 逐条记回填命中同一读数——报告据此记数据日期，不把陈数据写成当日抓取。

缓存目录（`--cache-dir`／`SCHOLAR_SEARCH_CACHE_DIR`，缺省 `<包目录>/.cache`）须落在被忽略的非交付路径：默认值满足，`workspace/<交付>/…` 或任何跟踪路径不满足（清理口径见 `workspace-guide.md` §15）。

检索格载荷为 `{ candidates, totalAvailable }`——日期过滤前的候选，加唯一不可由候选复算的读数（实返、日期计数、`capHit`、`capPolicy` 在命中时由载荷复算，故命中格不丢截断披露）。`CACHE_VERSION` 不变：更严格的载荷守卫让旧形态格（裸候选数组）走既有「形状对不上即 miss」通路，重取后按当前形态改写，不写兼容分支。命中格 `attempts`／`retried`／`recoveredByRetry` 记 `0`／`false`／`false`，`collectedAt` 记原始采集时刻。

#### 取数加固

所有 provider 请求经同一道闸门，瞬时故障不会悄悄缩格，被截断的格也不会读成完整。

- **节流与并发上限** — PubMed 串行 ≤3 rps（实测 3 并发即全 429）、arXiv 单连接 ≤1 请求/3 秒（ToU）、OpenAlex 与 Crossref 10 rps、Semantic Scholar 与 SerpApi 1 rps；重试与 cap 补偿的请求同样排队。
- **同格有界重试** — 每格就地重跑一次，只对 `unavailable`（传输失败或 5xx）与 `quota_exhausted`（429）；`invalid_request`（400）与 `auth_failed`（401/403）不重试。退避 800–1600ms（base 800ms 含 jitter），格内串行、不整池重跑、不进缓存键。
- **总数诊断** — 源不报总数时 `totalAvailable` 记 `null`（不得以 0 或实返数冒充）；`totalAvailable > hitCount` 时 `capHit` 为 `true`，总数不可得记 `null`——`null` 不得读作 `false`。
- **分源类型的 cap 处置** — 精确集源（PubMed、arXiv）先抬 limit 至 `min(totalAvailable, 30)`，带窗格仍超则按年切片（最旧年优先、≤6 片、片内 ≤30），剩余缺口进账；召回形源（OpenAlex、Crossref、Semantic Scholar、Google Scholar）只记总数、不加请求。`capPolicy` ∈ `none`／`unknown`／`recorded_only`／`raised_limit`／`year_sliced`。
- **日期窗** — 无合法 `YYYY-MM-DD` 的候选保留入池并标 `sourceMetadata.windowStatus = "date_unknown"`；日期合法但落窗外者丢弃并计数（`dateRejectedCount`）；无窗格全部记 `unfiltered`。合并簇只要有一个成员有日期且在窗内，`date_unknown` 标记即消失。
...
| `SCHOLAR_SEARCH_CACHE_DIR` | 任意供应商 | 响应缓存目录；`--cache-dir` 优先 |
