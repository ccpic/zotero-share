...
test("enrich fills missing abstracts from a provider detail lookup", async () => {
  const adapter: ProviderAdapter = { id: "openalex", search: async () => ({ candidates: [candidate()], totalAvailable: null }), detail: async () => candidate({ abstract: "Enriched abstract." }) }
...
test("enrich keeps existing abstracts without issuing a detail lookup", async () => {
  const adapter: ProviderAdapter = { id: "openalex", search: async () => ({ candidates: [candidate({ abstract: "Already present." })], totalAvailable: null }) }
  const response = await executeSearch({ queries: ["q"], providers: ["openalex"] }, { runtime, adapters: { openalex: adapter }, enrich: true })
...
test("enrich with a provider lacking detail logs info and leaves results unchanged", async () => {
  const adapter: ProviderAdapter = { id: "openalex", search: async () => ({ candidates: [candidate()], totalAvailable: null }) }
  const response = await executeSearch({ queries: ["q"], providers: ["openalex"] }, { runtime, adapters: { openalex: adapter }, enrich: true })
...
test("enrich isolates a failing detail lookup with a warning diagnostic", async () => {
  const bad: ProviderAdapter = { id: "openalex", search: async () => ({ candidates: [candidate()], totalAvailable: null }), detail: async () => { throw new Error("detail boom") } }
  const good: PaperCandidate = { ...candidate(), externalId: "W2", doi: "10.1234/w2", title: "Paper Two", abstract: "Second abstract." }
  const goodAdapter: ProviderAdapter = { id: "crossref", search: async () => ({ candidates: [good], totalAvailable: null }) }
...
test("offline enrichment completes the interpretability abstract via OpenAlex", async () => {
  const adapter: ProviderAdapter = { id: "openalex", search: async () => ({ candidates: [candidate({ externalId: "W4391101111", doi: "10.5555/llm.2024.001", title: "Interpretability in Large Language Models", abstract: null })], totalAvailable: null }), detail: openAlexProvider.detail }
