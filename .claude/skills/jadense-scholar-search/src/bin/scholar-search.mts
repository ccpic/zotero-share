...
type CliOptions = SearchRequest & { offline: boolean; enrich: boolean; format: "json" | "markdown"; userAgent: string; contactEmail?: string; cacheDir: string; noCache: boolean; refresh: boolean }
...
function parseArgs(argv: string[]): CliOptions {
  const queries: string[] = []; const selected: SearchProviderId[] = []; let limit: number | undefined; let from: string | undefined; let to: string | undefined; let offline = false; let enrich = false; let format: CliOptions["format"] = "json"; let userAgent = process.env.SCHOLAR_SEARCH_USER_AGENT?.trim() || "scholar-search-cli/0.1.0"; let contactEmail = process.env.SCHOLAR_SEARCH_CONTACT_EMAIL?.trim() || process.env.CONTACT_EMAIL?.trim() || undefined; let cacheDir = process.env.SCHOLAR_SEARCH_CACHE_DIR?.trim() || DEFAULT_CACHE_DIR; let noCache = false; let refresh = false
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (flag === "--cache-dir") cacheDir = resolve(read())
    else if (flag === "--no-cache") noCache = true
  ...
  return { queries, providers: selected.length ? [...new Set(selected)] : undefined, limit, dateRange, offline, enrich, format, userAgent, contactEmail, cacheDir, noCache, refresh }
}

function markdown(response: Awaited<ReturnType<typeof executeSearch>>): string {
  const collected = response.queryPlan.queryStatuses.map((cell) => `${cell.provider} "${cell.query}" ${cell.collectedAt?.slice(0, 10) ?? "no data"}${cell.cacheHit ? " (cache hit)" : ""}`).join("; ")
  const enrichmentDates = [...new Set(response.queryPlan.enrichCacheHits.map((hit) => hit.collectedAt.slice(0, 10)))]
  const enrichment = enrichmentDates.length ? `; enrichment cache ${response.queryPlan.enrichCacheHits.length} hit(s) collected ${enrichmentDates.join(", ")}` : ""
  const lines = ["# Scholarly search results", "", `Providers: ${response.queryPlan.providers.join(", ")}`, `Queries: ${response.queryPlan.queries.join(" / ")}`, `Results: ${response.results.length}`, `Data collected: ${collected || "none"}${enrichment}`, ""]
...
function printHelp() { process.stdout.write(`jadense-scholar-search\n\nUsage:\n  node dist/bin/scholar-search.mjs --query <text> [options]\n\nOptions:\n  -q, --query <text>       Query; repeat up to four times\n  -p, --provider <id>      Provider; repeat or comma-separate\n      --limit <1..30>      Retrieval limit\n      --from <YYYY-MM-DD>  Inclusive publication date\n      --to <YYYY-MM-DD>    Inclusive publication date\n      --format <json|markdown>
      ...
      --cache-dir <path>    Provider-response cache directory (default ${DEFAULT_CACHE_DIR})
...
export async function runCli(argv = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const cache = options.noCache ? undefined : createFileCache(options.cacheDir, { onIssue: (message) => process.stderr.write(`scholar-search cache: ${message}\n`) })
    const response = await executeSearch({ queries: options.queries, providers: options.providers, limit: options.limit, dateRange: options.dateRange }, { offline: options.offline, refresh: options.refresh, cache, enrich: options.enrich ? { abstract: true, providers: ["openalex"] } : undefined, runtime: { userAgent: options.userAgent, contactEmail: options.contactEmail, credentials: { serpApiKey: process.env.SERPAPI_API_KEY?.trim() || undefined, semanticScholarApiKey: process.env.SEMANTIC_SCHOLAR_API_KEY?.trim() || undefined, ncbiApiKey: process.env.NCBI_API_KEY?.trim() || undefined, ncbiTool: process.env.NCBI_TOOL?.trim() || undefined } } })
