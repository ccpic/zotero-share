...
import { requestText, totalOrNull } from "./common.js"
...
export async function searchArxiv(input: ProviderSearchInput): Promise<ProviderSearchResult> {
  const query = normalizeText(input.query)
  if (!query) return { candidates: [], totalAvailable: null }
  if (input.offline) return { candidates: (OFFLINE_RESULTS[query.toLowerCase()] ?? []).slice(0, input.limit).map((item, index) => mapOffline(item, query, index)).filter((item): item is PaperCandidate => Boolean(item)), totalAvailable: null }
  ...
  if (!root) return { candidates: [], totalAvailable: null }
  // `<opensearch:totalResults>` 是该查询在 arXiv 索引里的条目数（精确集源），与本次实返条数无关。
  return { candidates: elements(root, "entry").map((entry, index) => mapEntry(entry, query, index)).filter((item): item is PaperCandidate => Boolean(item)), totalAvailable: totalOrNull(text(first(root, "opensearch:totalResults"))) }
}

export const arxivProvider: ProviderAdapter = {
  id: "arxiv",
  // 精确集源：`opensearch:totalResults` 是索引内条目数，cap 命中走抬 limit／按年切片（submittedDate 窗）。
  sourceType: "exact_set",
  search: searchArxiv,
  detail: async ({ candidate, runtime, offline }) => { const id = normalizeExternalId("arxiv", candidate.externalId); return id ? (await searchArxiv({ query: `id:${id}`, limit: 1, runtime, offline })).candidates[0] ?? null : null },
}
