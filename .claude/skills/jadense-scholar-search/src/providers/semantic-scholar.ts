...
export async function searchSemanticScholar(input: ProviderSearchInput): Promise<ProviderSearchResult> {
  const query = normalizeText(input.query); if (!query) return { candidates: [], totalAvailable: null }
  if (input.offline) return { candidates: query.toLowerCase() === "transformer interpretability" ? [mapPaper(OFFLINE_PAPER, query, 0)].filter((item): item is PaperCandidate => Boolean(item)) : [], totalAvailable: null }
  const payload = record(await request(new URL(buildSemanticScholarSearchUrl(input)), input)); const candidates = (Array.isArray(payload?.data) ? payload.data : []).map((item, index) => mapPaper(record(item) ?? {}, query, index)).filter((item): item is PaperCandidate => Boolean(item))
  // 检索面 `total` 字段名未验证（票 06 §2 探测时匿名池 429）：读到就记、读不到记 null，不猜不凑。
  return { candidates, totalAvailable: totalOrNull(payload?.total) }
}
