...
export async function searchCrossref(input: ProviderSearchInput): Promise<ProviderSearchResult> {
  const query = normalizeText(input.query)
  if (!query) return { candidates: [], totalAvailable: null }
  const doi = normalizeDoi(query)
  if (input.offline) return { candidates: (doi || query.toLowerCase() === "a mathematical framework for transformer circuits" ? [OFFLINE_WORK] : []).slice(0, input.limit).map((item) => mapMessage(item, query, 1)).filter((item): item is PaperCandidate => Boolean(item)), totalAvailable: null }
  const payload = record(await requestJson(crossrefUrl(input, doi ?? undefined).toString(), input.runtime, "crossref"))
  const message = record(payload?.message)
  const items = message?.items
  const messages: unknown[] = doi ? [message] : (Array.isArray(items) ? items : [])
  const candidates = messages.map((item, index) => mapMessage(record(item) ?? {}, query, doi ? 1 : Math.max(0.1, 1 - index * 0.03))).filter((item): item is PaperCandidate => Boolean(item))
  // `message.total-results` 是全文命中形（召回形源）；单件取件路径（DOI 查询）记 null。
  return { candidates, totalAvailable: doi ? null : totalOrNull(message?.["total-results"]) }
}

export const crossrefProvider: ProviderAdapter = {
  id: "crossref",
  // 召回形源：实测该计数达 35 万量级，与「被漏掉的相关文献」不同量纲（票 06 §2）。
  ...
  detail: async ({ candidate, runtime, offline }) => {
    ...
    return results.candidates[0] ?? null
  },
}
