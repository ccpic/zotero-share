...
import { record, requestJson, requestText, totalOrNull } from "./common.js"
...
export async function searchPubmed(input: ProviderSearchInput): Promise<ProviderSearchResult> {
  const query = normalizeText(input.query); if (!query) return { candidates: [], totalAvailable: null }
  if (input.offline) return { candidates: (OFFLINE_SUMMARIES[query.toLowerCase()] ?? []).slice(0, input.limit).map((item, index) => mapSummary(item, query, index)).filter((item): item is PaperCandidate => Boolean(item)), totalAvailable: null }
  const payload = record(await requestJson(buildPubmedSearchUrl({ ...input, query }), input.runtime, "pubmed"))
  const esearch = record(payload?.esearchresult)
  const idList = esearch?.idlist
  const ids = (Array.isArray(idList) ? idList : []).map((id) => normalizeText(id)).filter(Boolean)
  const candidates = (await summaries(ids, input)).map((item, index) => mapSummary(item, query, index)).filter((item): item is PaperCandidate => Boolean(item))
  // `esearchresult.count` 是检索集规模（精确集源）；这是 PubMed 唯一的总数来源，ID 转摘要后实返数不参与。
  return { candidates, totalAvailable: totalOrNull(esearch?.count) }
}
...
export const pubmedProvider: ProviderAdapter = {
  id: "pubmed",
  // 精确集源（票 03 §4）：检索集规模可受理地穷尽，cap 命中走抬 limit／按年切片。
